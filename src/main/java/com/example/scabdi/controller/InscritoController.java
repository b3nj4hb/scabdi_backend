...
public class InscritoController {
	...
	// CREAR
	@PostMapping("/create")
	public ResponseEntity<Inscrito> save(@RequestBody Inscrito us) {
		try {
			Inscrito ua = service.create(us);
	...
	// LISTAR
		@GetMapping("/all")
		public ResponseEntity<List<Inscrito>> list() {
			try {
		...
		// BUSCAR {ID}
		@GetMapping("/read/{id}")
		public ResponseEntity<Inscrito> search(@PathVariable("id") int id) {
			Inscrito us = service.read(id);
			if (us.getId() > 0) {
		...
		public ResponseEntity<Inscrito> update(@RequestBody Inscrito u, @PathVariable("id") int id) {
			try {
				Inscrito us = service.read(id);
				if (us.getId() > 0) {
