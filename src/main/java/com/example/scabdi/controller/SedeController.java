...
public class SedeController {
	...
	// CREAR
	@PostMapping("/create")
	public ResponseEntity<Sede> save(@RequestBody Sede us) {
		try {
			Sede ua = service.create(us);
	...
	// LISTAR
		...
		// BUSCAR {ID}
		@GetMapping("/read/{id}")
		public ResponseEntity<Sede> search(@PathVariable("id") int id) {
			Sede us = service.read(id);
			if (us.getId() > 0) {
		...
		public ResponseEntity<Sede> update(@RequestBody Sede u, @PathVariable("id") int id) {
			try {
				Sede us = service.read(id);
				if (us.getId() > 0) {
